...
export async function POST(request: NextRequest) {
    try {
        ...
        // prev vote does not exist or voteStatus is different
        if(response.documents[0]?.voteStatus !== voteStatus) {
            ...
            // Increase or decrease the reputation of the user
            const questionOrAnswer = await databases.getDocument(
                db,
                type === "question" ? questionCollection : answerCollection,
                typeId
            ...
            if(response.documents[0]){
                await users.updatePrefs<UserPrefs>(questionOrAnswer.authorId, {
                    // means prev vote was "upvoted" and now it is "downvoted" so decrease the reputation
                    reputation: response.documents[0].voteStatus === "upvoted" ? Number(authorPrefs.reputation) - 1 : Number(authorPrefs.reputation) + 1,
                });
            } else {
                await users.updatePrefs<UserPrefs>(questionOrAnswer.authorId, {
